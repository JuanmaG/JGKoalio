...
public class ayudaScreen implements Screen {

    private Stage stage;
    private Game game;

    public ayudaScreen(Game aGame) {
        game= aGame;
        stage = new Stage(new ScreenViewport());

        Label title = new Label("ESPACIO Y FLECHAS PA JUGAAAAA", MyGdxGame.gameSkin);
        title.setAlignment(Align.center);
        title.setY(Gdx.graphics.getHeight()/2);
        title.setWidth(Gdx.graphics.getWidth());
        stage.addActor(title);
        TextButton playButton = new TextButton("Volver",MyGdxGame.gameSkin);
